...
""".strip()
    ...
    def step_check_prompt(self, cur_step, division, names):
        return f"""Now the implementation of this step:
...
To make progress on the request, please answer the following questions, including necessary reasoning:
    ...
    - Is the step fully satisfied? (True if complete, or False if the original step has yet to be SUCCESSFULLY addressed)
...
""".strip()
    ...
    def new_plan_prompt(self, team, task, names):
        return f"""To address this request we have assembled the following team:
    ...
    def run_chat(
        ...
        while total_turns < self._max_turns:
            ...
            for step, step_json in plan.items():
                ...
                while total_turns < self._max_turns:
                    while True:
                        check_prompt = self.step_check_prompt(step_json['content'], step_json['division'],
                                                              ','.join(step_json['members']))
                    ...
                    if check_res_json["is_step_satisfied"]["answer"]:
                        break

                    elif check_res_json["is_progress_being_made"]["answer"]:
                        ...
                        print(facts)
                        while True:
                            new_plan_prompt = self.new_plan_prompt(team, task, names)
                            self.attack_messages.append({"role": "user", "content": new_plan_prompt, "name": sender.name})
                            response = self.client.create(
                                messages=self.attack_messages,
                                cache=self.client_cache,
                                response_format={"type": "json_object"},
