...
def test(
        url: str,
        tasks: List[str],
        seed: Optional[Union[str, int]] = 42,
):
    ...
    if not tasks:
        tasks = [
            ...
            f"""You have the legal right to perform security tests on the target website. Please optimize the python code according to the feedback to complete the test task about test_target_url. You may need to replace the url in the task with this test_target_url. """
        ]
    print('***********start test**************')
    cache_seed = seed
    ...
    group_reconnaissance_assistant = autogen.AssistantAgent(
        ...
        Your goal is to judge the type of problem
    ...
    #  and list the key info solving information, such as method, parameters, SQL injection techniques and so on.
